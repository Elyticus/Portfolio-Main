...
const SAMPLE_SIZE = 300 // px grid the photo is sampled on
const ROW_STEP = 3 // scan-line spacing
...
const DEPTH_DOME = 42 // face-dome extrusion strength
const DEPTH_LUMA = 26 // luminance extrusion strength
const CAMERA_Z = 400

const PALETTE = {
  dark: { a: 0x3fd6a3, b: 0x4aa8d8, lineOpacity: 0.85, pointOpacity: 0.9, additive: true },
  light: { a: 0x0f766e, b: 0x1d6f94, lineOpacity: 0.8, pointOpacity: 0.85, additive: false },
}
...
function samplePortrait(img: HTMLImageElement) {
  ...
  for (let y = 0; y < SAMPLE_SIZE; y += ROW_STEP) {
    ...
    for (let x = 0; x < SAMPLE_SIZE; x += COL_STEP) {
      ...
      const lum = lumAt(x, y)
      row.push({ x: x - half, y: half - y, z: depthAt(x, y, lum), lum, edge: edgeAt(x, y) })
    }
    rows.push(row)
  }
  return rows
...
export default function HeroHologram({ theme }: { theme: Theme }) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      // Scan-line segments between adjacent foreground samples
      const linePos: number[] = []
      const lineCol: number[] = []
      // Sparse highlight points on bright pixels
      ...
      for (const row of rows) {
        for (let i = 0; i < row.length - 1; i++) {
          ...
          if (!a) continue
          if (b) {
            // Facial features (edges) etch brighter than flat areas so the
            // likeness reads clearly; light theme inverts the shading curve
            // because features must darken, not glow, on a white background
            const shade = (s: Sample) => {
              tmp.copy(colorA).lerp(colorB, 1 - s.lum / 255)
              const featureBoost = Math.min(s.edge / 140, 1)
              const gain = palette.additive
                ? 0.1 + 0.75 * Math.pow(s.lum / 255, 1.4) + featureBoost * 0.6
                : 0.35 + 0.5 * (1 - s.lum / 255) + featureBoost * 0.5
              return [tmp.r * gain, tmp.g * gain, tmp.b * gain]
            }
            linePos.push(a.x, a.y, a.z, b.x, b.y, b.z)
            lineCol.push(...shade(a), ...shade(b))
          }
          if (a.edge > 60 && i % 2 === 0) {
            pointPos.push(a.x, a.y, a.z + 1.5)
            tmp.copy(colorA).lerp(colorB, 1 - a.lum / 255)
            pointCol.push(tmp.r, tmp.g, tmp.b)
          }
        }
      }
